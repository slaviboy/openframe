...
import { fontOutlinesPlugin } from './scripts/vite-plugin-font-outlines';

export default defineConfig({
  // Relative URLs: the build works from any path (Live Server serving dist/, GitHub Pages at /Artboard/).
  base: './',
  plugins: [react(), serviceWorkerPlugin(), cjkSubsetsPlugin(new URL('.', import.meta.url)), fontOutlinesPlugin(new URL('.', import.meta.url))],
