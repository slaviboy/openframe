...
| Layout guides | ⇧G, zoom menu, Main menu → View; Layout guide section | Frames can have any number of layout guides (Grid, Columns, Rows), drawn over their contents and clipped to them while the frame is unrotated. The view option hides all of them (per device) and each guide has its own visibility; hidden guides still snap and still guide constraints. |
