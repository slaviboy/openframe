...
| Drag an arc handle (hover a selected ellipse) | A plain ellipse shows the sweep handle 12px inside its right-hand point. Dragging it around the ellipse sets `arcData` (`startingAngle`, `endingAngle` in radians clockwise from the right-hand point, `innerRadius` 0–1); passing the start switches between a full clockwise and a full counter-clockwise sweep, and the label shows the sweep as a percentage. Once the ellipse has a gap, the start handle (with a dot) turns the arc and the ratio handle, halfway along the sweep at the inner radius, sets the inner radius; dragged from the arc into its gap it switches `arcData` to the other segment (starting where the old one ended, turning the same way). [`arcCommands`](../src/core/geometry/arc.ts) outlines the arc for rendering, flattening, outline stroke and the selection outline, and `arcContains` hit-tests it. Hidden on locked layers and under 64px on screen; each drag is one undo step ([`arc-handles.ts`](../src/editor/interactions/arc-handles.ts)). |
...
- **Vector edit mode** (`vector.edit`, Return; or a Move-tool double-click): `editorState.vectorEdit` holds the layer and its selected points; `VectorEditController` ([`interactions/vector-edit.ts`](../src/editor/interactions/vector-edit.ts)) is routed as the canvas editor. Point drags run `moveVertices` from the drag's start and `refitVector` against the starting transform; double-clicking a path runs `splitSegment` at `nearestOnSegments`; `vector.deletePoints` (registered before layer delete) runs `deleteVertices`. `vector.healPoints` (⇧Delete) runs `healVertices`: a point joining exactly two segments is removed and they become one segment between their far ends, keeping the outer handle directions lengthened to the joined length, with the region loops kept; other points are deleted with their segments. While editing, `Toolbar` shows the secondary toolbar: `vector.toolMove` (V) and `vector.toolLasso` (Q), registered first so they act only while editing, set `vectorEdit.tool`; with the Lasso the controller collects the dragged screen outline (drawn by the overlay as `vectorLasso`) and selects the points inside it by the even-odd rule, Shift adding to the selection, and Done runs `vector.done`. `vector.toolCut` (X) picks Cut: clicking a point runs `cutVertex`, which gives every other segment end at the point its own copy of it and drops regions whose loops used those segments; clicking a path first adds a point there with `splitSegment`. Dragging it instead runs `cutAlongLine` ([`vector-divide.ts`](../src/core/vector/vector-divide.ts)), which splits each segment where it crosses the dragged line and cuts through the new points; `splitComponents` separates the pieces, the one with the first point staying in the layer and the others becoming vector layers of their own directly above (the overlay draws the line as `vectorCutLine`). Regions the drag crosses are rebuilt by `divideNetwork` from `GeometryService.regionHalves` (the region's area intersected with each side of the line, with CanvasKit path ops), keeping their fills; without the engine their outlines are cut open. With two or more points selected and the Move tool, [`pointsFrame`](../src/editor/interactions/vector-points-frame.ts) wraps them in a selection frame in the layer's space, so `hitHandle`, `hitRotationCorner` and `drawHandles` work as they do for layers: an edge or corner drag runs `computeResize` (Shift keeps proportions, Alt from the center) and `scalePoints`, and a drag from just outside a corner runs `rotatePoints` about the box's center (Shift snaps to 15°) ([`vector-transform-points.ts`](../src/core/vector/vector-transform-points.ts)); a point under the pointer takes priority over the box. With Bend (toolbar only), pressing on a point — or on a path, after `splitSegment` adds one — and dragging runs `bendVertex` ([`vector-bend.ts`](../src/core/vector/vector-bend.ts)): the segment end leaving the point takes the drag as its tangent and the other end the opposite. Selected points draw their handles (`selectedHandles`); pressing on one drags that tangent with `setTangent`, the opposite end following when the two were mirrored when the drag started. Shift-clicking handles selects them (`vectorEdit.selectedHandles`), and dragging one of several selected handles moves them all by the same offset (`moveHandles`). With Paint (⇧B), pressing on a closed region (`regionAt` in [`vector-paint.ts`](../src/core/vector/vector-paint.ts)) sets its `fills` to the tool's paint (`vectorEdit.paint`, edited in the Paint section; by default the layer's first solid fill), or to `[]` when its fills already equal that paint; a drag makes the same change to each region it crosses. The renderer paints a region's own fills in place of the layer's, and the overlay stripes the region under the pointer (`vectorPaintHover`). With the Eraser (⇧E), each pointer move erases along the whole path so far from the network the drag started with ([`eraseNetwork`](../src/core/vector/vector-erase.ts)): `eraseOpenSegments` clips segments that bound no region, and each region the stroke reaches is rebuilt from `GeometryService.regionMinusStroke` (`SceneRenderer`: the region's area minus a round stroke, with CanvasKit path ops), keeping the region's fills; the weight (`vectorEdit.eraserWeight`, 10 by default) is set in the Eraser section. With Variable width (toolbar only), [`vector-width.ts`](../src/core/vector/vector-width.ts) follows the stroke of a network without branches (`strokeChain`): clicking the stroke adds a width point (`strokeWidths`: a position along the path and a full width) at the current width, snapped to a vector point or a midpoint (`snapPosition`; Ctrl places it exactly); dragging one of its knobs (at least 12px from the path) sets the width from the pointer's distance, dragging the point moves it along the path, Shift-click adds to the selection (`vectorEdit.widthPoints`), Delete removes the selected points, and the Width point section edits their width. The renderer fills `variableWidthOutline` — each side offset by half the width eased between points (`widthAt`) — with the stroke paints in place of the uniform stroke. `drawVectorEdit` draws the segments and points. Return (`vector.done`), Escape, clicking outside the layer or changing the selection leaves.
...
- **Instances**: `duplicateNodes` and paste turn copies of a main component into instances ([`instantiate`, `pastedInstanceLayer`](../src/core/document/instances.ts)): the instance frame gets `instance.mainId` and each layer `source` (the main component's layer it mirrors). `componentFinalizer` runs first among the history finalizers: sets on main-component layers are copied to their instance layers unless listed in the layer's `overrides` (the instance's own placement is never copied; size only while it still matched), sets on instance layers record an override when the field is in `OVERRIDABLE_FIELDS` ([`instance-fields.ts`](../src/core/document/instance-fields.ts)) and are reverted otherwise.
- **Detach instance** (`object.detachInstance`, ⌥⌘B): [`detachInstances`](../src/editor/commands/detach.ts) clears the frame's `instance` and every layer's `source` and `overrides`; the sync finalizer ignores these link fields, so the detached frame keeps its current layers and its layers can then move freely.
- **Reset all changes** (`object.resetOverrides`): [`resetOverrides`](../src/core/document/instances.ts) copies the main component's value back for every field in `overrides` and clears the list; the sync finalizer also drops a field from `overrides` when an edit makes it equal to the main component's value again.
- **Main component commands** ([`main-component.ts`](../src/editor/commands/main-component.ts)): `object.goToMainComponent` (⌃⌥⌘K) selects `instance.mainId` and runs `view.zoomToSelection`; `object.restoreMainComponent`, when the main component is gone, clones the instance (without its links, marked `component`) 40px to its right and points the instance's `instance.mainId` and each layer's `source` at the clones; `object.pushChangesToMain` sets every overridden field's value on the matching main-component layer (the sync finalizer passes it on) and clears the instance's `overrides`.
- **Quick insert** (`view.quickInsert`, ⇧I): opens [`CommandPalette`](../src/ui/palette/CommandPalette.tsx) in `components` mode, whose entries are `localComponents` and whose choice runs `insertInstance`.
- **Assets tab** (`view.assetsTab`, ⌥2; `editorState.assetsOpen`): [`AssetsPanel`](../src/ui/panels/assets/AssetsPanel.tsx) lists `localComponents` and inserts with [`insertInstance`](../src/editor/commands/insert-instance.ts) — on click next to the main component, or on the canvas where a drag carrying `COMPONENT_DRAG_TYPE` is dropped (`CanvasHost`).
- **Component configuration**: the Inspector's Component section edits `component.description` and `component.link` through [`setComponentConfiguration`](../src/editor/commands/components.ts) (trimmed; empty removes; one undo step on blur); instances show their main component's values, and links open only when `isSafeLink` (http/https). `localComponents` carries descriptions so the Assets tab and Quick insert search them.
- **Create component** (`object.createComponent`, ⌥⌘K): [`createComponent`](../src/editor/commands/components.ts) marks a single selected frame with `component` (a frame field holding the description and link), or nests any other selection with `wrapSelection(…, 'FRAME')` in a fill-less frame named "Component N" and marks that. Components are frames, so layout, clipping, rendering and hit testing apply unchanged; `layerIcon` shows the component icon and the properties panel labels them Component.
- **Outline stroke** (`object.outlineStroke`, ⌘⌥O): [`outlineStrokeSelection`](../src/editor/commands/outline-stroke.ts) asks the editor's `GeometryService` (installed by the canvas host; implemented by `SceneRenderer.strokeOutline` with `makeDashed`, `makeStroked`, `MakeFromOp` and `makeSimplified`) for each layer's stroke area, converts it with `commandsToNetwork` into a `VECTOR` filled with the stroke paints at the layer's transform, and replaces the layer — or, when it has a visible fill, clears its strokes and places the vector directly above it.
