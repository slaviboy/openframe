...
- **With a selection:**
  ...
  - Layout: W, H (H disabled for lines), Constrain proportions, "space between" for smart selections, and Clip content for frames
    ...
    - `createAutoLayoutFinalizer` runs after the constraints and text finalizers at commit, and in preview mode while a gesture is open (frames whose children are only being moved are skipped, so a dragged child follows the pointer until release). It lays out affected frames deepest first with `layoutFlow` ([`core/layout/flow-layout.ts`](../src/core/layout/flow-layout.ts)), re-laying out parents of hugging frames that resized and auto layout children resized by fill. Constraints skip children of auto layout frames.
