...
| `componentPropertyReferences` | `{ visible?, characters?, mainComponent? }` (optional) | Inside a main component or variant: the component property, by name, that the layer's visibility (a boolean property), text (a text property, text layers only) or component (an instance swap property, nested instances only) follows. Instance layers copy it from their main component's layers. |
