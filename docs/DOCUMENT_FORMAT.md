...
| `effects` | `Effect[]` (optional, ≤ 64) | Shadows and blurs, absent when there are none. `DROP_SHADOW` / `INNER_SHADOW`: `color` (alpha = opacity), `offset {x, y}`, `radius` (blur, ≥ 0), `spread`, `visible`, `blendMode`; drop shadows add `showShadowBehindNode`. `LAYER_BLUR` / `BACKGROUND_BLUR`: `radius`, `visible`. |
