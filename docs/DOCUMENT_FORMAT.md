...
| `componentPropertyReferences` | `{ visible?, characters? }` (optional) | Inside a main component or variant: the component property, by name, that the layer's visibility (a boolean property) or text (a text property, text layers only) follows. Instance layers copy it from their main component's layers. |
