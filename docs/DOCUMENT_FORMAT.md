...
| `constraints` | `{ "horizontal", "vertical" }` (optional) | How the layer responds when its parent frame is resized. Each axis is `MIN` (left/top), `MAX` (right/bottom), `STRETCH` (both edges; the layer resizes), `CENTER` or `SCALE` (position and size proportional). Absent means `MIN` on both axes. |
| `layoutSizingHorizontal`, `layoutSizingVertical` | `"HUG"` \| `"FILL"` (optional) | Resizing in auto layout: `HUG` on auto layout frames fits them to their content; `FILL` on children of auto layout frames takes the free space along the flow (shared by content area) or the full space across it. Absent means fixed. Text hugs through `textAutoResize` and stores only `FILL`. |
| `minWidth`, `maxWidth`, `minHeight`, `maxHeight` | number ≥ 0 (optional) | Size limits applied by auto layout to auto layout frames and their children (a frame still never gets smaller than its padding). A text layer has either `maxHeight` or `maxLines`. |
| `layoutPositioning` | `"ABSOLUTE"` (optional) | Ignore auto layout: the child of an auto layout frame stays out of the flow, keeps its position and follows its `constraints`. |
| `gridColumnSpan`, `gridRowSpan`, `gridColumn`, `gridRow`, `gridChildHorizontalAlign`, `gridChildVerticalAlign` | optional | Children of grid auto layout frames: spans (integers ≥ 1, absent means 1), the 0-based cell used with manual positioning, and alignment within the cell (`CENTER` \| `MAX`; absent is left and top). |
...
| `componentPropertyReferences` | `{ visible?, characters?, mainComponent?, slot? }` (optional) | Inside a main component or variant: the component property, by name, that the layer's visibility (a boolean property), text (a text property, text layers only) or component (an instance swap property, nested instances only) follows, or the slot property a frame is the slot of. Instance layers copy it from their main component's layers. |
| `fillStyleId`, `strokeStyleId`, `effectStyleId`, `gridStyleId` (frames), `textStyleId` (text) | style id (optional) | The local style a layer's fills, strokes, effects, layout guides or typography come from while it is applied. Changing that property on the layer directly detaches the style; the layer keeps the values. |
| `boundVariables` | map (optional) | Variables bound to the layer's properties (`opacity`, `visible`, `width`, `height`, `minWidth`, `maxWidth`, `minHeight`, `maxHeight`, `cornerRadius`, `strokeWeight`, `itemSpacing`, `counterAxisSpacing`, padding sides, and on text `characters`, `fontFamily`, `fontStyle`, `fontSize`, `lineHeight`, `letterSpacing`, `paragraphSpacing`, `paragraphIndent`) as variable aliases. The properties hold the resolved values; changing one directly removes its binding. A solid paint's color binds with the paint's own `boundVariables.color`. On an instance of a variant, `variant:<property>` binds a variable (string, number or boolean) to a variant property: the instance uses the variant whose value for the property matches the variable's value. |
| `exportSettings` | array (optional) | Export configurations from the Export section, in order: `format` (`PNG`, `JPG` or `WEBP`), `suffix` (added to the file name) and `constraint` (`SCALE` with a multiplier, or `WIDTH` or `HEIGHT` with a size in pixels). Exported files are named from the layer name, slash-separated parts becoming folders. |
| `explicitVariableModes` | map (optional, layers and pages) | Variable modes by collection id. A layer uses the nearest mode set on itself, its containers or its page (Auto), else the collection's default mode. A mode set for an extended collection applies to its parent's variables, with the extended collection's overriding values. |
...
| `VARIABLE` | A variable in a collection: `resolvedType` (`COLOR`, `FLOAT`, `STRING` or `BOOLEAN`), `valuesByMode` (per mode id, a value of that type or `{type: 'VARIABLE_ALIAS', id}` following another variable of the same type; aliases never form cycles), optional `description`, `scopes` (the properties it is offered for; absent means all) and `codeSyntax` (`WEB`, `ANDROID`, `iOS` names). |
| `FRAME` | `clipsContent: boolean`; optional `component: { description?, link? }` (a main component), `componentSet: { description?, link? }` (a component set: its main component children are variants, named `Property1=value, Property2=value`), `componentPropertyDefinitions` (main components and component sets: their component properties by name in creation order, each `{ "type": "BOOLEAN", "defaultValue": boolean }`, `{ "type": "TEXT", "defaultValue": string }`, `{ "type": "INSTANCE_SWAP", "defaultValue": componentId, "preferredValues"?: componentId[] }` or `{ "type": "SLOT", "description"?, "preferredValues"?: componentId[], "minLayers"?, "maxLayers"?, "onlyPreferred"?: true, "showEmpty"?: true, "fillCounterAxis"?: true }`), `instance: { mainId }` (an instance, whose layers link to the main component's layers by `source`) and `isExposedInstance: true` (a nested instance in a main component whose component properties the component's instances show with their own); optional `guides: Guide[]` in the frame's local space (used for frames on the page or in a section); optional `layoutGuides: LayoutGuide[]`; auto layout: optional `layoutMode` (`HORIZONTAL` \| `VERTICAL` \| `GRID`; absent is freeform) lays out visible children in layer order, with optional `layoutWrap: true` (horizontal only), `paddingTop`/`paddingRight`/`paddingBottom`/`paddingLeft` (≥ 0), `itemSpacing` (gap along the flow, may be negative), `counterAxisSpacing` (gap between wrapped rows), `primaryAxisAlignItems` (`CENTER` \| `MAX`, or an Auto gap `SPACE_BETWEEN` \| `SPACE_AROUND` \| `SPACE_EVENLY`; absent is the start) and `counterAxisAlignItems` (`CENTER` \| `MAX` \| `BASELINE`, which lines up text layers' first baselines in horizontal flows; absent is the start), `itemReverseZIndex: true` (canvas stacking: the first child is drawn and hit-tested on top, without changing layer order) and `strokesIncludedInLayout: false` (inside strokes don't take up room); the grid flow uses `gridColumnSizes` (one track per column) and optional `gridRowSizes` (absent: Auto rows, as many 1fr rows as the cells need), where a track is `{ "type": "FIXED", "value" }` (pixels), `{ "type": "FLEX", "value" }` (fr) or `{ "type": "HUG" }`, plus optional `gridColumnGap`, `gridRowGap` and `gridAutoPositioning: false` (children keep their `gridColumn`/`gridRow` cells instead of filling cells in layer order). Children's positions and fill sizes, and hugging frames' sizes, are kept laid out by the editor at every commit; inside strokes count as padding. A boolean or text component property can have `boundVariables.defaultValue`, a variable alias its default value follows (a boolean or string variable for boolean properties, a string or number variable for text properties), resolved in the component's variable modes. |
