...
| `FRAME` | `clipsContent: boolean`; optional `guides: Guide[]` in the frame's local space (used for frames on the page or in a section); optional `layoutGuides: LayoutGuide[]`; auto layout: optional `layoutMode` (`HORIZONTAL` \| `VERTICAL`; absent is freeform) lays out visible children in layer order, with optional `layoutWrap: true` (horizontal only), `paddingTop`/`paddingRight`/`paddingBottom`/`paddingLeft` (≥ 0), `itemSpacing` (gap along the flow, may be negative), `counterAxisSpacing` (gap between wrapped rows), `primaryAxisAlignItems` (`CENTER` \| `MAX`, or an Auto gap `SPACE_BETWEEN` \| `SPACE_AROUND` \| `SPACE_EVENLY`; absent is the start) and `counterAxisAlignItems` (`CENTER` \| `MAX`; absent is the start), `itemReverseZIndex: true` (canvas stacking: the first child is drawn and hit-tested on top, without changing layer order) and `strokesIncludedInLayout: false` (inside strokes don't take up room). Children's positions and fill sizes, and hugging frames' sizes, are kept laid out by the editor at every commit; inside strokes count as padding. |
