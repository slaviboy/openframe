...
What is left is the backfill the user asked for on 2026-09-17: the rows still In progress or Planned in the earlier milestones. 162 rows are Implemented and 44 are still open (40 in progress, 4 planned), counting the ones recorded as a browser limitation or a local equivalent as done. Work them largest block first, since that is where the most is missing:

- **M1 — 11 rows.** The biggest gap now.
- **M6 — 8 rows:** 109 (sketching in white on dark canvases), 110 (Pen: snapping, the rubber-band line, adding to existing vectors), 111 (Space to reposition, editing shapes other than vectors, several layers at once), 112 (the Bend tool's mirroring modes), 113 (gradient paints and droplet cursors, the Eraser's shape), 114 (width profiles, outlining a variable-width stroke, hit testing along it), 115 (Shape builder, nothing built), 116 (text inside a boolean group) and 118 (outlining frames, boolean groups, per-side weights, arrowheads, longer dash patterns), 121 (the Simplify slider, lasso point selection).
- **M5 — 7 rows.**
- **M2 — 5 rows:** 37 (pixel preview), 45 (a UI for configurable bindings), 48 (selecting hidden layers in outline mode, stroke placement detail, "Include object bounds"), 77 (smart selection: resizing within the selection, 2D grids and the ⌘ swap), 92 (snapping for rotated resizes, matching gaps elsewhere on the canvas).
- M9 (3), M4 (2), M8 (1), and four rows spanning two milestones each.

**A fidelity pass is owed after these.** Every panel was built from the documentation's prose; its screenshots (1,468 of them in the mirror's `assets/img/`, ordinary PNGs) were never opened, so the layouts are invented. The user raised this about Dev Mode on 2026-09-17 and chose to finish the feature rows first. They also offered to save real the reference pages as HTML to align against, which would be better than the screenshots.
