...
What is left is the backfill the user asked for on 2026-09-17: the rows still In progress or Planned in the earlier milestones. Of the 214 rows, 207 are Implemented (counting the ones recorded as a browser limitation or a local equivalent as done) and **7 are still open**:

- **Row 37 (M2)** — pixel preview in the zoom menu. Nothing built.
- **Row 78 (M1)** — an accessible representation of the canvas. Nothing built.
- **Row 111 (M6)** — vector edit mode across several layers at once, which is also what the Shape builder needs to reach past one layer.
- **Row 116 (M6)** — text inside a boolean group: the shape is built while a frame is drawn, so glyph outlines would have to be worked out and cached ahead of it.
- **Row 256 (M6 / M3)** — Draw mode's own vector tools. Nothing built.
- **Row 298 (M1)** — large documents (10,000 nodes): frame-rate profiling and tile caching.
- **Row 300 (M9)** — the remaining SVG parts and the format-specific export settings.
