...
What is left is the backfill the user asked for on 2026-09-17: the rows still In progress or Planned in the earlier milestones. 158 of the 229 rows are Implemented; 48 are still open once the ones recorded as a browser limitation or a local equivalent are set aside. Work them largest block first, since that is where the most is missing:
