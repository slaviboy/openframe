...
- **Fonts:** the bundled Inter variable font (weight axis 100–900, upright and italic) loads with CanvasKit before the canvas reports ready ([`bundled-fonts.ts`](../src/engine/text/bundled-fonts.ts)). The font files are embedded as base64 in a lazily imported chunk (`bundled-font-data.ts`) and decoded in memory, because app code never uses `fetch`, not even for same-origin assets.
  ...
  - **Color emoji:** Noto Color Emoji (5.7 MB) is embedded in its own chunk (`emoji-font-data.ts`). CanvasHost loads it through `loadEmojiFont` once any text layer contains emoji (`containsEmoji`), and registers it with `TextShaper.registerFallbackFonts`, which drops cached layouts. After any font registers (CJK subsets, the emoji font, user fonts), `Editor.refitText` refits every auto-sized text layer in a transaction committed with `undoable: false`, so boxes measured before the glyphs existed take their real size without adding an undo step.
    - Offline: the chunk isn't part of the service worker's install precache. Once the canvas is ready, CanvasHost calls `precacheDeferredAssets`, and the worker then caches the build's deferred list (`PRECACHE_DEFERRED`) one file at a time. So the first install doesn't download it alongside the CanvasKit wasm, but it is available offline after the app has started once.
