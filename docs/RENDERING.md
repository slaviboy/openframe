...
- **Laid-out text is kept by the frame that drew it.** `TextShaper.beginFrame`, called once per frame by the canvas host and presentation view, sweeps the blocks the frame before it didn't draw once there are more than 256 of them — a frame's own working set is never swept, however large. Past `CACHE_CEILING` blocks a frame shapes and throws away instead of keeping, so the native paragraphs a page holds stay bounded. See [PERFORMANCE.md](PERFORMANCE.md#text-what-a-frame-keeps-and-what-it-doesnt-draw-at-all) for what this costs when it goes wrong.
- **Text too small to read is drawn as bars, not glyphs.** With `RenderOptions.greekText` on — the canvas and presentation view, never an export or a thumbnail — a layer under three device pixels to the em draws as one bar per line (`TextShaper.greekedLines`, `SceneRenderer.drawGreekedText`) and is not shaped at all. Lines fall where the font's ascent, descent and leading put them, each as wide as its share of the layer's characters; a bar is drawn in the layer's own fills at `GREEK_INK` (0.38) of their alpha, which is about what Latin glyphs cover of the line box they sit in. Text on a path is never greeked: its lines don't sit in its box.
- **Fonts:** the bundled Inter variable font (weight axis 100–900, upright and italic) loads with CanvasKit before the canvas reports ready ([`bundled-fonts.ts`](../src/engine/text/bundled-fonts.ts)). The font files are embedded as base64 in a lazily imported chunk (`bundled-font-data.ts`) and decoded in memory, because app code never uses `fetch`, not even for same-origin assets.
  ...
  - **CJK:** Noto Sans SC, TC, JP and KR come from the Fontsource variable packages. Each is split into about 100–124 WOFF2 subsets by Unicode range.
    - The `openframe-cjk-subsets` Vite plugin ([`scripts/vite-plugin-cjk.ts`](../scripts/vite-plugin-cjk.ts)) reads each package's `wght.css` at build time and serves `virtual:cjk-subsets/<script>`: subset index plus hex ranges.
    - [`engine/text/cjk-fonts.ts`](../src/engine/text/cjk-fonts.ts) globs the subset files as lazily imported `?inline` modules. `loadCjkSubsets(script, text, requested)` decodes only the subsets covering the text's non-ASCII characters (`cjkSubsetsFor`) that haven't been requested yet. CanvasHost calls it for every text layer with CJK characters after each change.
    - `TextShaper.registerCjkSubsets` registers them as `Noto Sans <script> (<n>)`. Those families are left out of every run's fallback list: listing all subsets made layout take hundreds of milliseconds. `cjkFallbacksFor(node)` appends only the loaded subsets of the layer's script (`cjkScriptFor`: the picked Noto family, else kana → JP, Hangul → KR, else SC).
    - `availableFonts()` lists the four families, and `fontAxes` gives them the weight axis.
  - **Color emoji:** Noto Color Emoji is read from the library under `public/fonts/google/noto-color-emoji/`, in the eleven subsets it is split into (2.0 MB together, 10 KB to 709 KB each). CanvasHost calls `readEmojiSubsets(text, requested)` ([`ui/fonts/google-fonts.ts`](../src/ui/fonts/google-fonts.ts)) for every text layer with emoji (`containsEmoji`) after each change; it picks the subsets covering the text from each file's `unicodeRange` (`subsetsFor`, [`core/text/font-subsets.ts`](../src/core/text/font-subsets.ts)) and registers them as `Inter (noto-color-emoji-<n>)` with `TextShaper.registerFallbackFonts`, which drops cached layouts. After any font registers (CJK subsets, emoji subsets, symbol fallbacks, user fonts), `Editor.refitText` refits every auto-sized text layer in a transaction committed with `undoable: false`, so boxes measured before the glyphs existed take their real size without adding an undo step.
    - It must be the library's copy: Fontsource's subsets of the same font carry no `COLR`/`CPAL` table, so CanvasKit paints nothing at all from them while their `cmap` still reports a glyph. See docs/FONTS.md.
    - Offline: these are ordinary library files, cached by the service worker's fetch handler the first time they are asked for, like the CJK subsets and every picked font family.
  - **Symbols:** arrows, mathematical operators, technical marks, box drawing, geometric shapes and dingbats come from three more library files (`SYMBOL_FALLBACK_FILES`, ~580 KB), read once `TextShaper.uncoveredCodePoints` finds a character no registered typeface has a glyph for. `registerSymbolFallbacks` keeps them out of every run's fallback list for the same reason the CJK subsets are kept out; `symbolFallbacksFor(node)` appends them only to layers whose text has symbols (`containsSymbols`).
...
- **Paragraphs:** each paragraph (text between line breaks) is its own SkParagraph ([`core/text/paragraphs.ts`](../src/core/text/paragraphs.ts)), laid out at a common width and stacked with `paragraphSpacing` between them.
  ...
  - **Hanging quotes** (`hangingPunctuation`): a paragraph starting with an opening quote gets a negative `firstLineShift` equal to the quote glyph's width, when it is left-aligned or justified, left to right and not a list item.
    - SkParagraph has no per-line offset, so `drawParagraphLines` draws the paragraph twice: clipped above the second line's top at the shifted x, and clipped below it at the normal x.
    - `lineLeft(layout, line)` gives each line's x, which carets, hit testing, selection rectangles, line navigation and underline pieces use instead of the layout's `left`.
