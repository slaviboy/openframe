...
/** Icon for a specific layer: masks show the mask icon, layers with a video fill the video icon, rectangles filled with an image the image icon. */
export function layerIcon(node: SceneNode): IconName {
  ...
  if ('fills' in node && node.fills.some((p) => p.type === 'VIDEO' && p.visible)) return 'video';
