...
export class PresentationRenderer {
  ...
  private loadTextFonts(): void {
    ...
    for (const node of this.editor.doc.nodes()) {
      ...
      if (containsEmoji(node.characters)) {
        readEmojiSubsets(node.characters, requestedEmoji)
          .then((fonts) => {
            if (this.disposed || !this.shaper || fonts.length === 0) return;
            this.shaper.registerFallbackFonts(fonts);
            this.invalidate();
          })
          .catch((error: unknown) => console.warn(error));
      }
  ...
  draw(scene: PresentedScene | null, background: Color, hints: readonly Rect[], scroll: ReadonlyMap<Id, Vec2> = new Map()): void {
    ...
    for (const item of scene?.items ?? []) {
      if (item.kind === 'device') {
        // The device: a body in its model's color with a thin edge (unless its frame is hidden), and its screen.
        if (item.frame) {
