    ...
    if (hasGeometry(node)) {
      for (const field of ['fills', 'strokes'] as const) {
        ...
        node[field].forEach((paint: Paint, index) => {
          if (!paint.visible || paint.type === 'IMAGE' || paint.type === 'VIDEO' || paint.type === 'PATTERN') return;
...
export function updateSelectionColor(tx: Transaction, usages: readonly PaintUsage[], edit: (paint: ColorPaint) => Paint): void {
  ...
  for (const [id, list] of byLayer) {
    ...
    for (const field of ['fills', 'strokes'] as const) {
      ...
      const next = node[field].map((paint, i) => (indices.includes(i) && paint.type !== 'IMAGE' && paint.type !== 'VIDEO' && paint.type !== 'PATTERN' ? edit(paint) : paint));
