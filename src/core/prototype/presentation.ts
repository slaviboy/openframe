...
export type PresentedItem =
  ...
  | { readonly kind: 'device'; readonly body: Rect; readonly bodyRadius: number; readonly screen: Rect; readonly screenRadius: number; readonly bodyColor: Color; readonly edgeColor: Color; readonly frame: boolean };
...
export interface DeviceScreen {
  ...
  /** Whether the device's frame (its body) shows, or only its screen. */
...
export function composeScene(store: DocumentStore, state: PlayerState, viewport: Size, scaling: ScalingMode, scrollY: number, playing: PlayingTransition | null, device: DeviceScreen | null = null): PresentedScene {
  ...
  return {
    ...
    items: [{ kind: 'device', body: device.body, bodyRadius: device.bodyRadius, screen: device.screen, screenRadius: device.screenRadius, bodyColor: device.bodyColor, edgeColor: device.edgeColor, frame: device.frame }, ...inScreen],
