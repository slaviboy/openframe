...
describe('cut tool in vector edit mode', () => {
  test('X picks Cut; clicking a path breaks it there into two selected ends, as one undo step', () => {
    expect(editState()?.tool).toBe('cut');
    click(150, 100);
    expect(network().vertices).toHaveLength(6);
    expect(network().vertices[4]).toEqual({ x: 50, y: 0 });
    expect(network().vertices[5]).toEqual({ x: 50, y: 0 });
    expect(network().regions).toEqual([]);
    expect(editState()?.vertices).toEqual([4, 5]);
    editor.history.undo();
    expect(network().vertices).toHaveLength(4);
    expect(network().regions).toHaveLength(1);
  });

  test('clicking a point breaks the path at the point; clicking off the path only clears the point selection', () => {
    click(200, 200);
    expect(network().vertices).toHaveLength(5);
    expect(editState()?.vertices).toEqual([2, 4]);
    click(600, 600);
    expect(editState()).toMatchObject({ tool: 'cut', vertices: [] });
    expect(network().vertices).toHaveLength(5);
  });
});
